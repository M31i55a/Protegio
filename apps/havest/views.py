...
def dashboard(request):
    ...
    return render(request, 'havest/dashboard.html', {
        'sessions': sessions,
        'stats': stats,
...
def scan_results(request, session_id):
    ...
    return render(request, 'havest/results.html', {
        'session': session,
        'result':  result,
    })
...
def delete_scan(request, session_id):
    ...
    return render(request, 'havest/confirm_delete.html', {'session': session})


@login_required
def scans_list(request):
    sessions = ScanSession.objects.all()
    return render(request, 'havest/scans_list.html', {'sessions': sessions})  # ← CORRECT